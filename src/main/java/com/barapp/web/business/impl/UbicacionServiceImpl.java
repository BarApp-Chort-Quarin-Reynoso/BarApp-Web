...
public class UbicacionServiceImpl implements UbicacionService {
    ...
    public String getMapUrl(Ubicacion ubicacion) {
        return "https://www.google.com/maps/embed/v1/place?key=".concat(ConfigClass.getInstance().MAPS_API_KEY).concat("&q=").concat(getUbicacionParameter(ubicacion));
    }

    public Pair<Double, Double> getLatitudeLongitud(Ubicacion ubicacion) {
        String query = "https://maps.googleapis.com/maps/api/geocode/json?address=".concat(getUbicacionParameter(ubicacion)).concat("&key=").concat(ConfigClass.getInstance().MAPS_API_KEY);
